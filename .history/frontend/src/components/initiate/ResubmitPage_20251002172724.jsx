...
export default function ResubmitPage() {
  ...
  // Only allow initiated and sent back tabs
  useEffect(() => {
    if (currentTab && !['initiated', 'sentback'].includes(currentTab)) {
      navigate('/approvals');
    }
  }, [currentTab, navigate]);

  const effectiveTab = useMemo(() => {
    if (['initiated', 'sentback'].includes(currentTab)) return currentTab;
  ...
  const viewKey = `${id}:${effectiveTab}:${isApprover ? 1 : 0}`;
  ...
  return (
    <Box sx={{ width: "100%", maxWidth: 1600, mx: "auto" }}>
      ...
      <InitiateForm
        ...
        hideActions={false}
        mode={isApprover ? "approver" : "initiator"}
        showButtons={settings.showButtons}
        tabKey={effectiveTab}
        requestId={id}
        onCancel={() => navigate(-1)}
        onApproverDone={() => navigate("/approvals?tab=assigned")}
        onOpenPath={() => setOpenPath(true)}
        showPathButton={settings.showApprovalPathBtn}
      />
    </Box>
  );
}

import InitiateForm from "../forms/InitiateForm";
  ...
  const [approvalFormIds, setApprovalFormIds] = useState({ fundRequestId: null, approvalId: null });
    if (tab === "assigned") {
      setApprovalFormIds({ fundRequestId, approvalId: row.approvalId ?? row.id });
      setShowApprovalForm(true);
    } else if (tab === "initiated" || tab === "sentback") {
      ...
      {/* Approval/Resubmit Form Modal for Assigned Tab */}
      {showApprovalForm && tab === "assigned" && (
        <Box sx={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", bgcolor: "rgba(0,0,0,0.4)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <Box sx={{ minWidth: 400, maxWidth: 700, bgcolor: "background.paper", borderRadius: 2, boxShadow: 3, p: 2 }}>
            <InitiateForm
              fundRequestId={approvalFormIds.fundRequestId}
              approvalId={approvalFormIds.approvalId}
              mode="approver"
              showButtons={{ approve: true, sentBack: true, reject: true, approveWithModification: true }}
              onAction={() => { setShowApprovalForm(false); setApprovalFormIds({ fundRequestId: null, approvalId: null }); }}
              readOnlyFields={[]}
            />
            <Box mt={2} textAlign="right">
              <Button onClick={() => setShowApprovalForm(false)} color="secondary">Close</Button>
            </Box>
          </Box>
        </Box>
      )}
...
// normalize list row for the table
const normalizeRow = (r) => {
  ...
  // pick nested fundRequest.id if present
...
const isValidTab = (t) => Object.prototype.hasOwnProperty.call(TAB_TITLES, t);
...
function adaptTrailForDialogDto(dto, listRow) {
  ...
  return {
    ...
    requestStatus:
      normalizeLooseStatus(dto?.requestStatus ?? dto?.RequestStatus ?? listRow?.status ?? "Pending"),
    initiatorName,
    steps: outSteps,
  };
}
...
export default function ApprovalsPage() {
  ...
  const [tab, setTab] = useState(() => {
    const initial = (params.get("tab") || "assigned").toLowerCase();
    return isValidTab(initial) ? initial : "assigned";
  });
  ...
  useEffect(() => {
    const urlTab = (params.get("tab") || "assigned").toLowerCase();
    const safe = isValidTab(urlTab) ? urlTab : "assigned";
    if (safe !== tab) setTab(safe);
  }, [params, tab]);

  // ensure URL has valid ?tab and reset table state on tab change
  useEffect(() => {
    const current = (params.get("tab") || "").toLowerCase();
    if (tab !== current) setParams({ tab }, { replace: true });
    setPage(0);
    setSearch("");
    setStatus("");
    setSortModel([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        let data = [];
        if (tab === "approved") {
          data = await getApprovals("approved", token);
        } else {
          data = await getApprovals(tab, token);
        }
  ...
  // scope rows by tab intent first
  const tabScoped = useMemo(() => byTab(tab, rows), [tab, rows]);

  // filter + sort + paginate (client)
  // replace your filtered useMemo with this version
const filtered = useMemo(() => {
  const q = (search || "").toLowerCase();
  return rows.filter((r) => {
    const s = normalizeDbStatus(r.status);
    // do not apply status filter on the Approved tab
  ...
  const sorted = useMemo(() => {
    ...
    return [...filtered].sort((a, b) =>
      val(a) > val(b) ? 1 * dir : val(a) < val(b) ? -1 * dir : 0
    );
  }, [filtered, sortModel]);
 ...
 // trail API (expects fundRequestId) - correct route: /api/approvals/{fundRequestId}/trail
  const fetchTrail = async (fundRequestId) => {
    return getApprovalTrail(fundRequestId);
  };

  // handlers wired into columns
  const navigateToEdit = (row) => {
    const fundRequestId = row.fundRequestId ?? row.ref;
    if (tab === "initiated" || tab === "sentback") {
      navigate(`/resubmit/${fundRequestId}?tab=${tab}`);
    } else {
      const approvalId = row.approvalId ?? row.id;
      const qs = new URLSearchParams({ tab, ...(approvalId ? { approvalId: String(approvalId) } : {}) });
  ...
  const handleOpenTrail = async (row) => {
    setTrailRow(row);
    setTrailOpen(true);
    setTrailError("");
    setTrailData(null);

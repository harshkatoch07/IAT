...
function adaptTrailForDialogDto(dto, listRow) {
  ...
  const outSteps = rawSteps.map((s) => ({
    ...
    approverName: s.approverName ?? s.ApproverName ?? "",
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        let data = [];
        if (tab === "approved") {
          const [fullyApproved, approvedByMe] = await Promise.all([
            getApprovals("approved", token),
            getApprovals("approved", token),
          ]);
          const byKey = new Map();
          const push = (arr) => {
            for (const r of arr) {
              const key =
                ...
                r.Id ??
                null;
              if (key != null && !byKey.has(key)) byKey.set(key, r);
            }
          };
          push(fullyApproved || []);
          push(approvedByMe || []);

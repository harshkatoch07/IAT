...
// ---------------------------
 function InitiateFormImpl({
  ...
  const [selectedProj, setSelectedProj] = useState("");     // == workflowId
  ...
  const [formData, setFormData] = useState({
    ...
    recommend: "",
    reason: "",
    attachments: [], // Array of File or { id, name, url, isExisting }
    amount: "",
    approvalBy: "",
  });

  // Track which attachments are existing (from server) vs new (File)
  ...
  // legal entity options (fallback if API schema lacks options)
  const localLegalEntities = [
    { value: "Gera Developments Private Limited", label: "Gera Developments Private Limited" },
    { value: "Gera Properties Private Limited", label: "Gera Properties Private Limited" },
    { value: "Gera Real Estate Private Limited", label: "Gera Real Estate Private Limited" },
    { value: "Gera Resorts Private Limited", label: "Gera Resorts Private Limited" },
    { value: "Grandwell Properties and Leasing Private Limited", label: "Grandwell Properties and Leasing Private Limited" },
    { value: "Construction Portal Private Limited", label: "Construction Portal Private Limited" },
    { value: "Inkpot Films Private Limited", label: "Inkpot Films Private Limited" },
    { value: "Happy Face Investments Private Limited", label: "Happy Face Investments Private Limited" },
    { value: "Gera Housing Private Limited", label: "Gera Housing Private Limited" },
    { value: "Gera Holdings Private Limited", label: "Gera Holdings Private Limited" },
    { value: "Gera Projects Private Limited", label: "Gera Projects Private Limited" },
  ];

  const hasField = (k) => (schema?.fields || []).some((f) => f.key === k);
  const isReq = (k) => !!((schema?.fields || []).find((f) => f.key === k)?.required);
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const wfRes = await api.get("/workflow?initiatorOnly=true");
        const raw = Array.isArray(wfRes.data) ? wfRes.data : [];
        const normalize = (w) => ({
          workflowId: w.workflowId ?? w.WorkflowId ?? w.workFlowId ?? w.WorkFlowId ?? null,
          name: w.name ?? w.Name ?? "",
          description: w.description ?? w.Description ?? "",
        });
        setWorkflows(raw.map(normalize));
        setLoadError(null);
      } catch {
        setLoadError("Failed to load workflows");
        if (USE_MOCKS) setWorkflows(mockDB.workflows);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const common = {
        ...
        const initial = {};
        fields.forEach((f) => { initial[f.key] = (dynValues?.[f.key] ?? ""); });
        if (initial.additionalInfo === undefined) initial.additionalInfo = dynValues?.additionalInfo ?? "";
  ...
  const buildFieldsArray = () => {
    ...
    (schema?.fields || []).forEach((f) => {
      ...
      // Special handling for hyperlinkUrl: store as comma-separated string for backend
      if (f.key === "hyperlinkUrl" && Array.isArray(val)) {
        val = val.filter((v) => v && v.trim() !== "").join(",");
      }

...
request.CurrentLevel = finalLevel;
        ...
        public async Task<IActionResult> GetApprovals([FromQuery] string filter)
        {
            ...
            switch (f)
            {
                ...
                case "sentback":
                {
                    var sentBack = await _db.FundRequests
                        .Include(fr => fr.Workflow)
                        .Include(fr => fr.Department)
                        .Where(fr => fr.InitiatorId == userId && fr.Status == StatusSentBack)
                        .OrderByDescending(fr => fr.CreatedAt)
                        .Select(fr => new
                        {
                            ApprovalId    = (int?)null,
        ...
        public async Task<IActionResult> GetApprovalTrail(int fundRequestId, CancellationToken ct)
        {
            ...
            // Filter out final receivers from workflow steps
            var nonFinalSteps = steps.Where(s => !s.IsFinalReceiver);
           ...
           // Resolve final receivers via provider (handles explicit user & designation-based mapping)
var finalUsers = await _finalReceivers.GetFinalReceiversAsync(
    workflowId:  fr.WorkflowId,
    projectId:   null,               // If you want to scope by the request's Project/Dept, load them above and pass here
    departmentId:null,
    ct);

var finalReceiverDtos = new List<ApprovalTrailStepDto>();
if (finalUsers.Count > 0)
{
    var maxSeq = steps.Count == 0 ? 0 : steps.Max(x => x.Sequence ?? 0);
    var finalLevel = maxSeq + 1;

    var finalLevelApprovals = approvals.Where(a => a.Level == finalLevel).ToList();

    foreach (var u in finalUsers)
    {
        var a = finalLevelApprovals.FirstOrDefault(x => x.ApproverId == u.Id);

        finalReceiverDtos.Add(new ApprovalTrailStepDto
        {
            Sequence = finalLevel,
            StepName = "Final Receiver",
            DesignationId = u.DesignationId,          // requires u.DesignationId to be available on User
            DesignationName = u.DesignationName,      // requires u.DesignationName to be available on User
            ApproverUserId = u.Id,
            ApproverName = u.FullName,
            Status = a != null ? (a.Status ?? StatusPending)
                    : (fr.Status == StatusApproved ? StatusPending : "Upcoming"),
            AssignedAt = a?.ActionedAt,
